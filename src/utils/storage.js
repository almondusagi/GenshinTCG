...
export const DEFAULT_TAGS=[
  ...
  {id:'t_charge1',name:'チャージコスト1',score:0,groupId:null},
...
// ── Cost tag computation (action cards) ──────────────────────────────────────
export function computeCostTags(card){
  ...
  // 同一タグはコスト１と同数付与
  if(total>=2&&hasNonVoid) for(let i=0;i<total;i++) result.push('同一');
  return result;
}

// ── Character tag computation (HP + charge cost) ──────────────────────────────
export function computeCharacterTags(card){
  if(card.type!=='character') return [];
  const result=[];
  const hp=card.hp||0;
  for(let i=0;i<hp;i++) result.push('HP１');
  const cost=typeof card.cost==='number'?card.cost:0;
  for(let i=0;i<cost;i++) result.push('チャージコスト1');
  return result;
}
...
export function initDefaults(cards){
  ...
  for(const [id,card] of Object.entries(cards)){
    const tags=card.type==='character'?computeCharacterTags(card):computeCostTags(card);
...
// ── Backup / Restore ──────────────────────────────────────────────────────────
// Format is compatible with TournamentUploader import.
// If 'presets' key exists, TournamentUploader will offer to import them as meta decks.
export function exportBackup(){
  const data={
    ...
    groups:loadGroups(),
    tags:loadTags(),
    tagCombos:loadTagCombos(),
    calcAdjusters:loadCalcAdjusters(),
    cardTags:loadCardTags(),
    presets:loadPresets(),
  };
